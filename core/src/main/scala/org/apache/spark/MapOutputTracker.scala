...
private[spark] class MapOutputTrackerMaster(conf: SparkConf,
  ...
  def registerMapOutputs(shuffleId: Int, statuses: Array[MapStatus],
    ...
    val errorInfo = new mutable.HashMap[Int, Array[Int]]()

    for ((mapstatus, id) <- statuses.zipWithIndex; if mapstatus.getShuffleBlockIds != null) {
      val merge = mapstatus.getShuffleBlockIds.map(_.mapId).toSet
      if ((usedSet & merge).isEmpty) {
        usedSet ++= merge
      } else {
        errorInfo.put(id, merge.toArray)
      }
    }
    for ((riffleId, _) <- errorInfo) {
      if (usedSet.contains(riffleId)) {
        riffleToMapSet.add(riffleId)
      }
      else riffleToSingleSet.add(riffleId)
    }
    val restSet = allSet -- usedSet --riffleToMapSet
    restSet.foreach(num => statuses(num).toRiffleMapStatus(ShuffleBlockId(shuffleId, num, 0)))
    riffleToMapSet.foreach(num => statuses(num).toMapStatus)
      ...
      for (k <- keys) {
        ...
        if (v.nonEmpty) {
          for (ss <- v) {
            ...
            print("shuffleId = " + k + "location = " + ss.location + " merge array = " + str + "/n")
          }
        }
      }
    }
